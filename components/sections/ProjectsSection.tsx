...
import { FolderOpen, ExternalLink, Github, Figma, Calendar, Users, Code, Shield, Wifi, Brain, Cpu, Sparkles } from "lucide-react";
...
export default function ProjectsSection() {
  ...
  return (
    <SectionWrapper id="projects">
      ...
      {/* ========== REUSABLE MODAL ========== */}
      <Modal isOpen={isModalOpen} onClose={closeModal} title={selectedProject?.name} subtitle={selectedProject?.tagline} size="xl">
        {selectedProject && (
              ...
              <div className="flex flex-wrap gap-2">
                {selectedProject.techStack.map((tech, index) => (
                  <motion.span
                    ...
                    className="px-3 py-2 text-sm rounded-lg bg-gradient-to-br from-white/5 to-white/2 border border-white/10 hover:border-emerald-500/30 hover:bg-emerald-500/10 transition-all duration-200 text-white font-medium"
                  >
                    {tech}
                  </motion.span>
                ))}
              ...
              <div className="p-5 rounded-2xl bg-gradient-to-br from-white/[0.03] to-white/[0.01] border border-white/10 backdrop-blur-sm shadow-lg">
                ...
                <ul className="space-y-3">
                  {selectedProject.features.map((feature, index) => (
                    <motion.li key={index} initial={{ opacity: 0, x: -5 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.05 }} className="flex items-start gap-3 p-3 rounded-lg bg-white/2 hover:bg-white/5 transition-colors">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {selectedProject.learnings.map((learning, index) => (
                    <motion.div
                      ...
                      className="p-3 rounded-lg bg-gradient-to-br from-cyan-500/5 to-blue-500/5 border border-cyan-500/10 hover:border-cyan-500/20 transition-all"
                    >
